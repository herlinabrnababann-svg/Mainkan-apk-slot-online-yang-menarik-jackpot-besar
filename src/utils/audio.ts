// Web Audio API Synthesizer for APK Slot Online
class SoundEngine {
  ...
  // Symbol explosion / win tumble
  public playWinExplosion() {
    if (this.isMuted) return;
    this.init();
    if (!this.ctx) return;
  ...
  // Thunder / Multiplier Strike (Zeus petir)
  ...
  // Scatter hit sound (increasing suspense from scatter 1 to 4)
  public playScatter(index: number) {
    if (this.isMuted) return;
    this.init();
    if (!this.ctx) return;

    const baseFreqs = [440, 554.37, 659.25, 880];
    const freq = baseFreqs[Math.min(index, 3)];
    const now = this.ctx.currentTime;
  ...
  // Big Win / Sensational Win Fanfare
  public playBigWin() {
    if (this.isMuted) return;
    this.init();
    if (!this.ctx) return;
    ...
    const times = [0, 0.12, 0.24, 0.36, 0.52, 0.72];
    const now = this.ctx.currentTime;
